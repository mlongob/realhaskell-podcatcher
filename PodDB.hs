...
getPodcast dbh wantedId = do
    ...
    case res of
        ...
        [] -> return Nothing
        _ -> fail "Really bad error; more thn one podcast with ID"

convPodcastRow :: [SqlValue] -> Podcast
convPodcastRow [svId, svURL] =
    Podcast {castId = fromSql svId,
             castURL = fromSql svURL}
convPodcastRow x = error $ "Can't covert podcast row " ++ show x
